...
class NotificationQueryHandler:
    ...
    async def handle(self) -> list[str]:
        ...
        user_strategies = await self.user_reader.get_user_strategies_by_id(
            user_id
        )
        ...
        for strategy in user_strategies.strategies:
            ...
            for key, value in portfolio.items():
                ...
                if target_date_str in c_d:
                    ...
                    print(
                        f"Данные на целевую дату {target_date_str} для актива {key}: {c_d}"
                    )
                else:
                    print(
                        f"Данных на целевую дату {target_date_str} для актива {key} нет. Пропускаем..."
                    )
                    continue
                ...
                for el in value:
                    ...
                    if investment:
                        ...
                        if percent_last:
                            print(
                                f"Процентная разница за последние 7 дней для {el.get('name')}: {percent_last}"
                            )
                            percent_last = float(
                                percent_last.replace("%", "")
                            )  # Преобразуем в число
                            if percent_last > 3:
                        ...
                        print(
                            f"Инвестиция {el.get('name')} не найдена в данных на целевую дату."
                        )
