...
class TokenWhiteListServiceImpl(TokenWhiteListService):
    ...
    async def is_fingerprint_matching(
        ...
        if stored_fingerprint is None:
            logger.warning(
                "Fingerprint отсутствует в данных токена с jti: %s", jti
            )
            return False
    ...
    async def remove_tokens_except_current(
        ...
        """
        token_jtis = await self.redis.zrange(
            f"refresh_tokens:{user_id}", 0, -1
        )
        ...
        if tokens_to_remove:
            await self.redis.zrem(
                f"refresh_tokens:{user_id}", *tokens_to_remove
            )
    ...
    async def remove_token(self, jti: UUID) -> None:
        ...
        await self.redis.zrem(f"refresh_tokens:{user_id}", jti)
        logger.info(
            "Токен с jti: %s удалён из списка refresh_tokens:%s", jti, user_id
        )
