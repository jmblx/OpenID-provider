...
class StrategyRepo:
    ...
    async def save(
        self, strategy: Strategy, portfolio: dict, user_id: UserID
    ) -> UUID:
        ...
        if existing_association:
            ...
            )  # Возвращаем существующий ID, если ассоциация уже существует
        ...
        stmt = (
            insert(user_strategy_association_table)
            .values(
    ...
    async def update_user_portfolio(
        self, strategy_id: UUID, user_id: UUID, portfolio: dict
    ) -> None:
        """Обновление портфеля пользователя для стратегии"""
        stmt = (
            update(user_strategy_association_table)
            .where(
                and_(
    ...
    async def get_user_portfolio(
        ...
        stmt = select(user_strategy_association_table).where(
            and_(
        ...
        if not record:
            raise ValueError(
                f"Портфель пользователя {user_id} для стратегии {strategy_id} не найден."
            )
