...
class UserReaderImpl(UserReader):
    ...
    async def get_user_strategies_by_id(
        self, user_id: UserID
    ) -> UserStrategiesDTO:
        ...
        if not strategies:
            raise ValueError(
                f"Стратегии для пользователя {user_id.value} не найдены."
            )
        ...
        return UserStrategiesDTO(
            user_id=user_id.value, strategies=user_strategies
        )

    async def _get_user_strategy_association(
        self, strategy_id: UUID, user_id: UserID
    ):
        ...
        if not user_strategy:
            raise ValueError(
                f"Association not found for user {user_id.value} and strategy {strategy_id}"
            )
