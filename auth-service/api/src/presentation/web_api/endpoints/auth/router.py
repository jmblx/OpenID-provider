...
from application.auth.code_to_token.code_to_token_handler import CodeToTokenHandler, CodeToTokenCommand
from application.auth.invalidate_other_tokens.handler import InvalidateOtherTokensCommand, \
    InvalidateOtherTokensHandler
from application.auth.login_user.auth_user_handler import (
    AuthenticateUserHandler, AuthenticateUserCommand,
)
from application.common.token_types import Fingerprint, RefreshToken
...
@auth_router.post("/invalidate-others")
async def invalidate_others(handler: FromDishka[InvalidateOtherTokensHandler], refresh_token: FromDishka[RefreshToken], fingerprint: FromDishka[Fingerprint], response: Response) -> ORJSONResponse:
    command = InvalidateOtherTokensCommand(refresh_token=refresh_token, fingerprint=fingerprint)
